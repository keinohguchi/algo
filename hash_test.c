...
static int test_ohash(void)
{
	const struct test {
		...
		{
			.name		= "ohash: 1 entry with division by 37 hash method",
			.positions	= 2,
			.h1		= divisionby37,
			.h2		= divisionby31,
			.size		= 1,
		...
		{
			.name		= "ohash: 8 entries with division by 37 hash method",
			.positions	= 16,
			.h1		= divisionby37,
			.h2		= divisionby31,
			.size		= 8,
		...
		{
			.name		= "ohash: 16 entries with division by 37 hash method",
			.positions	= 32,
			.h1		= divisionby37,
			.h2		= divisionby31,
			.size		= 16,
		...
		{
			.name		= "ohash: 32 entries with division by 37 hash method",
			.positions	= 64,
			.h1		= divisionby37,
			.h2		= divisionby31,
			.size		= 32,
	...
	for (t = tests; t->name; t++) {
		...
		for (i = 0; i < t->size; i++) {
			...
			if (ret == -1) {
				fprintf(stderr, "%s: %d=ohash_remove(%d)\n",
					t->name, ret, t->data[i]);
				goto perr;
			}
			if (*got != t->data[i]) {
